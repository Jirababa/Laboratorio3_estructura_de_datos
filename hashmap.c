...
{   
    ...
    long Lugar = hash(key , map->capacity);
    long Primer = Lugar;

    while (map->buckets[Lugar] != NULL)
        {
            if (map->buckets[Lugar]->key != NULL)
            {
                if(is_equal(map->buckets[Lugar]->key , key))
                {
                    map->current = Lugar;
                    return map->buckets[Lugar];
                }
            }
...
Pair * firstMap(HashMap * map) 
{
    ...
    for (long GH = 0 ; GH < map->capacity ; GH++)
        {
            if (map->buckets[GH] != NULL && map->buckets[GH]->key != NULL)
            {
                map->current = GH;
                return (map->buckets[GH]);
            }
        }
...
Pair * nextMap(HashMap * map) 
{
    ...
    for (long JK = map->current + 1 ; JK < map->capacity ; JK++)
        {
            if (map->buckets[JK] != NULL && map->buckets[JK]->key != NULL)
            {
                map->current = JK;
                return (map->buckets[JK]);
            }
        }
...
{
    enlarge_called = 1; //no borrar (testing purposes) (Nose si debo borrar esto)

    Pair ** Bucket_Viejo = map->buckets;
    ...
    for (long CJ = 0 ; CJ < map->capacity ; CJ++)
        {
            map->buckets[CJ] = NULL;
        }
    ...
    for (long GP = 0 ; GP < Capacidad_Vieja ; GP++)
        {
